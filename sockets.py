...
class WebSocketHandler:
    ...
    def run(self):
        try:
            ...
            print(f"⚠️ WebSocket error ({self.user_id}): {e}\n {traceback.print_exc()}")
        finally:
            self.cleanup()

    def cleanup(self):
        print(f"🔌 Disconnect: {self.user_id}")
        for doc_id in self.joined_docs:
            if doc_id in self.clients and self.ws in self.clients[doc_id]:
                ...
                print(f"🔄 Removed {self.user_id} from {doc_id}")

    def handle_join_doc(self, message):
        doc_id = message.get("doc")
        if not doc_id:
            print("⚠️ joinDoc missing 'doc' field")
            return

        if doc_id in self.joined_docs:
            print(f"🔁 {self.user_id} already joined {doc_id}")
            return
        ...
        print(f"✅ Client {self.user_id} joined doc {doc_id}")
    ...
    def handle_leave_doc(self, message):
        doc_id = message.get("doc")
        if not doc_id:
            print("⚠️ leaveDoc missing 'doc' field")
            return
        ...
        if doc_id in self.joined_docs:
            ...
            if doc_id in self.clients and self.ws in self.clients[doc_id]:
                self.clients[doc_id].remove(self.ws)
                print(f"🚪 Client {self.user_id} left doc {doc_id}")
                if not self.clients[doc_id]:
                    del self.clients[doc_id]
        else:
            print(f"⚠️ Client {self.user_id} not in doc {doc_id}")

    def handle_updates(self, message):
        doc_id = message.get("doc")
        if doc_id not in self.joined_docs:
            print(f"⚠️ Updates received for unjoined doc {doc_id}")
            return
        ...
        print(f"updates for file {doc_id}: {doc_store.get_all_updates_for_doc(doc_id)}")
    ...
    def handle_resync_request(self, message):
        doc_id = message.get("doc")
        if doc_id not in self.joined_docs:
            print(f"⚠️ Resync requested for unjoined doc {doc_id}")
            return
    ...
    def send_init_for_doc(self, doc_id):
        try:
            ...
            mode = "collaborative"
            print(f"Mode for this document is: {mode}")
            base = {
                "type": "init",
                "doc": doc_id,
                "clientID": "system",
            ...
            broadcast_list = self.clients.get(doc_id, [])
            self.broadcast(broadcast_list, base)
            return current_version
        except Exception as e:
            print(f"❌ Failed to send init data for doc {doc_id}: {e}")
            return None
